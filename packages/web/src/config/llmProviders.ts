...
import type { LLMProviderSetting, LLMVendor } from '../types/ai/agent.type';
...
export const resolveLLMProvider = (config: LLMProviderSetting): LLMProviderSetting => {
  ...
  if (vendor === 'custom') {
    return { ...config, vendor, customHeaders: config.customHeaders.map(header => ({ ...header })) };
  }
  return { ...config, vendor, provider: 'OpenAICompatible', baseURL: llmPresets[vendor].baseURL, apiKey: config.apiKey.trim(), customHeaders: [], extraBody: '' };
};
// 校验配置并返回可翻译的提示
