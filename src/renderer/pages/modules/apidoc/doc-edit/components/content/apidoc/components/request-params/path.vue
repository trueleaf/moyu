    ...
    <s-collapse-card v-bind="$attrs">
        ...
        <div slot="operation" class="d-flex">
            ...
            <el-popover ref="jsonView" placement="right">
                <s-array-view :data="pathParams" class="w-500px mt-2">
                    <div v-copy="jsonPathParams" slot="header" class="cursor-pointer">复制为json</div>
                </s-array-view>
        ...
        <s-params-tree
            ...
            :nest="false"
            :mind-params="mindParams.pathParams"
            showCheckbox
        >
        </s-params-tree>
        <!-- 弹窗 -->
        <s-json-schema :visible.sync="dialogVisible" :mind-params="mindParams.pathParams" @success="handleConvertJsonToParams"></s-json-schema>
        <s-params-template :items="pathParams" type="pathParams" :visible.sync="dialogVisible3" @success="handleAddParamsTemplate"></s-params-template>
    </s-collapse-card>
</template>
...
export default {
    ...
    computed: {
        pathParams: { //请求参数
            get() {
                return this.$store.state.apidoc.apidocInfo?.item?.pathParams;
            },
            set(val) {
        ...
        templateList() { //参数模板列表
            return this.$store.state.apidoc.presetParamsList.filter((val) => val.presetParamsType === "pathParams");
        },
        jsonPathParams() {
            const pathParams = this.$store.state.apidoc.apidocInfo?.item?.pathParams;
            const convertPathParams = this.convertPlainParamsToTreeData(pathParams || []);
            return JSON.stringify(convertPathParams, null, 4);
        },
    },
    data() {
        return {
    ...
    methods: {
        ...
        handleConvertJsonToParams(result, convertType) {
            ...
            if (convertType === "append") {
                this.$store.commit("apidoc/unshiftPathParams", convertData)
            } else if (convertType === "override") {
                this.$store.commit("apidoc/changePathParams", convertData)
            }
        ...
        handleSelectPresetParams(template) {
            ...
            let currentLocalData = localStorage.getItem("apidoc/pathParamsTemplate") || "{}";
            ...
            localStorage.setItem("apidoc/pathParamsTemplate", JSON.stringify(currentLocalData));
            ...
            for (let i = 0, len = preParams.length; i < len; i += 1) {
                ...
                if (!this.pathParams.find((val) => val.key === element.key)) {
                    this.pathParams.unshift(element);
                    this.$refs.paramsTree.selectChecked()
                }
            }
        },
