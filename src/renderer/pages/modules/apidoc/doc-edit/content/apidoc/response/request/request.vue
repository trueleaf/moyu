...
<template>
  <div v-if="requestInfo.url" class="request-info" :class="{ vertical: layout === 'vertical' }">
    ...
    <s-collapse bold>
      ...
      <pre v-if="contentType === 'application/json'" class="pl-1 pre">{{ formatJsonStr(requestInfo.body as string) }}</pre>
      <div v-else-if="contentType?.includes('multipart/')" class="pl-1 pre">
        <pre>{{ formatFormadata() }}</pre>
      </div>
      <pre v-else-if="contentType === 'application/x-www-form-urlencoded'" class="pre">{{ requestInfo.body }}</pre>
      <pre v-else-if="contentType === 'text/html'" class="pre">{{ requestInfo.body }}</pre>
      <pre v-else-if="contentType === 'text/javascript'" class="pre">{{ requestInfo.body }}</pre>
      <pre v-else-if="contentType === 'text/plain'" class="pre">{{ requestInfo.body }}</pre>
      <pre v-else-if="contentType === 'application/xml'" class="pre">{{ requestInfo.body }}</pre>
    </s-collapse>
  </div>
