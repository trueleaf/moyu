...
const formatFormadata = () => {
  // todo
  const bufferFormData = (apidocRequestStore.body as FormData)?.getBuffer();
  const stringFormData = bufferFormData.toString();
  const boundary = (apidocRequestStore.body as FormData)?.getBoundary();
  const result: string[] = [];
